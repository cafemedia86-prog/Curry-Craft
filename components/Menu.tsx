...
const Menu: React.FC<MenuProps> = ({ onProductClick, searchQuery }) => {
  ...
  // If we have enough items in this category, show top 5. 
  // If not, we might show fewer or fallback to global featured (optional, here we stick to category context).
  ...
  return (
    <div className="pb-28">
                ...
                {featuredItems.map(item => (
                    ...
                    >
                        ...
                        <div className="px-1 mb-1">
                            ...
                            <p className="text-green-400/60 text-xs mt-1 capitalize">{item.category.replace('-', ' ')}</p>
                        </div>
                        
                        <div className="flex justify-between items-center px-1 mt-3">
                            <span className="text-amber-400 font-bold text-lg">₹{item.price}</span>
                            <div className="w-9 h-9 bg-[#022c22] rounded-full flex items-center justify-center text-white border border-green-800 hover:bg-amber-500 hover:text-[#022c22] hover:border-amber-500 transition-colors">
                                <Plus size={18} />
                            </div>
                        </div>
                    </div>
      ...
      <section className="px-5">
        {isSearching && (
             <h3 className="text-xl font-sans font-bold text-white tracking-wide mb-4">
                 {displayedItems.length > 0 ? `Results for "${searchQuery}"` : `No results for "${searchQuery}"`}
             </h3>
        )}
        
        {!isSearching && (
             <h3 className="text-xl font-sans font-bold text-white tracking-wide mb-4 animate-in fade-in duration-300">
                 {currentCategoryLabel}
             </h3>
        )}

        {displayedItems.length === 0 ? (
             <div className="flex flex-col items-center justify-center py-10 text-green-400/50">
                {isSearching ? (
                     <>
                    ...
                    >
                        ...
                        <div className="flex-1 flex flex-col justify-between py-1">
                            ...
                            <div className="flex justify-between items-center mt-2">
                                <span className="text-amber-400 font-bold text-lg">₹{item.price}</span>
                                <button className="w-8 h-8 bg-[#022c22] rounded-full flex items-center justify-center text-white border border-green-800 hover:bg-amber-500 hover:text-[#022c22]">
                                    <Plus size={16} />
                                </button>
                            </div>
                        </div>
