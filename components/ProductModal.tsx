    ...
    return (
        <div className="fixed inset-0 z-[100] bg-[#022c22] flex flex-col animate-in slide-in-from-bottom duration-300">
            ...
            <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center z-10">
                <button onClick={onClose} className="w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-all active:scale-90">
                    <ArrowLeft size={20} />
                </button>
                <div className="flex gap-4">
                    <button
                        onClick={() => toggleFavorite(item.id.toString())}
                        className={`w-10 h-10 backdrop-blur-md rounded-full flex items-center justify-center transition-all active:scale-90 ${isFav ? 'bg-red-500 text-white shadow-lg shadow-red-500/40' : 'bg-white/20 text-white hover:bg-white/30'
                            }`}
                    >
                    ...
                    <div className="flex gap-3">
                        {['Half', 'Full', 'Family'].map((w) => (
                            <button
                                ...
                                className={`flex-1 py-2 rounded-xl text-sm font-bold border transition-all ${selectedWeight === w
            ...
            <div className="bg-green-950 p-6 rounded-t-3xl border-t border-green-900 shadow-2xl flex items-center justify-between">
                ...
                <QuantitySelector item={item} size="lg" />
            </div>
        </div>
    );
};
